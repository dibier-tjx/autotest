...
class TJXG3PH:
    ...
    async def pv_close_to_sp(self, sp: float, timeout: float = 20.):
        ...
        tolerance = self._g3api.get_ph_tolerance()
        timeout = max(20., timeout)
        while True:
            elapsed_time = asyncio.get_running_loop().time() - start_time
            pv = await self._g3api.get_ph_pv()
    ...
    async def test_02(self):
        async with self._lock:
            assert await self._g3api.set_ph_sp_source('Fixed')
            for it in self._g3api.get_ph_sps():
                pv1 = await self._g3api.get_ph_pv()
                assert await self._g3api.set_ph_sp(it)
    ...
    async def test_03(self):
        profile = await self._g3api.set_ph_profile()
        if profile is not None:
            async with self._lock:
                assert await self._g3api.restart_runtime()
                assert await self._g3api.set_ph_sp_source('Profile')
                for it in profile:
                    pv1 = await self._g3api.get_ph_pv()
                    ...
                    if elapsed_time is not None:
                        ...
                        await self._g3api.set_ph_sp_source('Fixed')
                        assert False
                await self._g3api.set_ph_sp_source('Fixed')