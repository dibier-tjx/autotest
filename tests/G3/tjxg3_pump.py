    ...
    async def test_01(self):
        async with self._lock:
            for i, pump in enumerate(self._g3api.get_pumps()):
                assert await self._g3api.set_pump_sp_source(i, 'Fixed')
                assert await self._g3api.set_pump_sp(i, 20.)
                assert await self._g3api.set_pump_sp_source(i, 'Disable')
                elapsed_time, _, pv = await self.pv_close_to_sp(i, 0.)
                if elapsed_time is not None:
                    allure.attach(body=f'Pump{i+1}[{pump}] curr pv is {pv}', name='Comment', attachment_type=allure.attachment_type.TEXT)
                    assert True
                else:
                    allure.attach(body=f'Pump{i+1}[{pump}] curr pv is {pv}, expect is zero', name='Error', attachment_type=allure.attachment_type.TEXT)
                    assert False
        ...
        async with self._lock:
            for i, pump in enumerate(self._g3api.get_pumps()):
                ...
                for it in self._g3api.get_pump_sps(i):
                    ...
                    assert await self._g3api.set_pump_sp(i, it)
    ...
    async def test_03(self):
        async with self._lock:
            for i, pump in enumerate(self._g3api.get_pumps()):
                if pump != 'Feed':
                    continue
                profile = await self._g3api.set_pump_profile(i)
                if profile is not None:
                    assert await self._g3api.restart_runtime()
                    assert await self._g3api.set_pump_sp_source(i, 'Profile')
                    for it in profile:
                        pv1 = await self._g3api.get_pump_pv(i)
                        elapsed_time, timeout, pv2 = await self.pv_close_to_sp(i, it[1], (it[0] + 0.01) * 3600)
                        if elapsed_time is not None:
                            allure.attach(body=f'Pump{i+1}[{pump}] {pv1}->{it[1]} speed {elapsed_time} s, curr pv is {pv2}', name='Comment', attachment_type=allure.attachment_type.TEXT)
                        else:
                            allure.attach(body=f'Pump{i+1}[{pump}] {pv1}->{it[1]} speed {timeout} s, curr pv is {pv2}', name='Error', attachment_type=allure.attachment_type.TEXT)
                            await self._g3api.set_pump_sp_source(i, 'Disable')
                            assert False
                    await self._g3api.set_pump_sp_source(i, 'Disable')
                    await asyncio.sleep(3)